...
group by productionDate  ";
        ...
        private const string GetAllShipped = @"SELECT {select_options} Pipe.number,  PipeMillSizeType.type, pipeMillStatus as " + ColumnNameForMillReport + @", PurchaseOrder.number, PurchaseOrder.date,wallThickness as wallThickness, weight,Pipe.length,Pipe.diameter,Plate.number, Heat.number, Pipe.isActive
              FROM Pipe 
              LEFT  JOIN PipeMillSizeType ON (PipeMillSizeType.id = Pipe.typeId)
              LEFT  JOIN PurchaseOrder ON (PurchaseOrder.id = Pipe.purchaseOrderId) 
              LEFT  JOIN Plate ON (Plate.id = Pipe.plateId)
        ...
        private const string GetAllProduced = @"SELECT {select_options} Pipe.number,  PipeMillSizeType.type, pipeMillStatus as " + ColumnNameForMillReport + @", PurchaseOrder.number, PurchaseOrder.date,wallThickness as wallThickness, weight,Pipe.length, Pipe.diameter,Plate.number, Heat.number, Pipe.isActive
            FROM Pipe 
            LEFT  JOIN PipeMillSizeType ON (PipeMillSizeType.id = Pipe.typeId)
            LEFT  JOIN PurchaseOrder ON (PurchaseOrder.id = Pipe.purchaseOrderId) 
            LEFT  JOIN Plate ON (Plate.id = Pipe.plateId)
        ...
        public const string GetPipelinePieces =
          ...
          @"SELECT id, number, N'Pipe' as type, diameter, wallThickness as wallThickness, length,'' as componentTypeName, constructionStatus 
            FROM pipe 
            WHERE isActive = 1 AND isAvailableToJoint = 1 AND isCutOnSpool = 0 AND [inspectionStatus] = 'Accepted'
            
            UNION ALL

            SELECT s.id, s.number, N'Spool' as type, p.diameter,p.wallThickness as wallThickness, p.length,'' as componentTypeName, s.constructionStatus 
            FROM spool s 
            ...
            SELECT c.id, c.number, N'Component' as type, con.diameter, con.wallThickness as wallThickness, c.length, ct.name as componentTypeName, c.constructionStatus 
            FROM component c 
