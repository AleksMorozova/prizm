...
namespace PrizmMain.Forms.Reports.Mill
{
    class SQLQueryString
    {
        public const string GetAllPipesByStatus = @"select Pipe.wallThickness as wallThickness, Pipe.diameter as diameter, 
          ...
          left  join TestResult TestResult on (TestResult.id = PipeTestResult.testResultId) where Pipe.pipeMillStatus=@status";
        ...
        public const string GetAllPipesOnMill = @"select Pipe.wallThickness as wallThickness, Pipe.diameter as diameter, 
          ...
          left  join PipeTestResult PipeTestResult on (PipeTestResult.pipeId = Pipe.id)
          left  join PurchaseOrder PurchaseOrder on (PurchaseOrder.id = Pipe.purchaseOrderId)
          left  join TestResult TestResult on (TestResult.id = PipeTestResult.testResultId)";
